...
module LeftLemma
{
    ...
    ghost predicate in_out_circuit(g: Graph, uc: seq<nat>)
        requires validUndirectedGraph(g)
        requires |g|>2 && |g|%3==0
        ...
        // requires isUndirectedHamiltonianCircuit(g, uc)
    {
        ...
        forall i :: 0<i<|uc|-1 ==>
        (
            (0<=uc[i]<|g|/3 ==>  //case og node
            (
                uc[i-1]==uc[i]+(|g|/3)*2  //before: in
                &&
                uc[i+1]==uc[i]+|g|/3    //next: out
            ))
            &&
            (|g|/3<=uc[i]<(|g|/3)*2 ==>  //case out node
            (
                uc[i-1]==uc[i]-|g|/3   //before: og
                &&
                (|g|/3)*2<=uc[i+1]<|g|   //next: in (range)
            ))
            &&
            ((|g|/3)*2<=uc[i]<|g| ==>  //case in node
            (
                |g|/3<=uc[i-1]<(|g|/3)*2   //before: out (range)
                &&
                uc[i+1]==uc[i]-(|g|/3)*2    //next: og
            ))
        )
    }
